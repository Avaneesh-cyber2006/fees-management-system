...
# Import the Selenium-based WhatsApp service
import sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'core'))
from whatsapp_service import send_whatsapp_message as send_via_selenium, cleanup_whatsapp_session
...
def send_whatsapp_message(mobile, message, student_name, registration_no, fees_amount, message_type='', days_diff=0, due_date=''):
    ...
    try:
        ...
        # Send message using Selenium service
        result = send_via_selenium(formatted_mobile, message, wait_time=15)
        
        if not result.success:
            log_message(student_name, registration_no, formatted_mobile, 'FAILED', fees_amount, message_type, days_diff, due_date, result.error)
            print(f"❌ Failed to send message to {student_name}: {result.error}")
            return False
...
Time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        send_via_selenium(MY_NUMBER, confirmation_msg, wait_time=10)
...
Pillay Sir's ICSE Classes - Automated System"""
        ...
        time.sleep(10)
        send_via_selenium(MY_NUMBER, summary_msg, wait_time=10)
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        print(f"⚠️ Critical error in send_reminders: {e}")
    finally:
